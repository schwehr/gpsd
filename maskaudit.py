...
import sys, subprocess, glob, getopt
...
class SourceExtractor:
    def __init__(self, sourcefile, clientside):
        ...
        for line in open(self.sourcefile).readlines():
    ...
    def in_library(self, flag):
        (status, _output) = subprocess.getstatusoutput("grep %s libgps_core.c libgps_json.c gpsctl.c" % flag)
        return status == 0

    def in_daemon(self, flag):
        (status, _output) = subprocess.getstatusoutput("grep %s %s" % (flag, " ".join(self.daemonfiles)))
        return status == 0
