...
""" + opts.GenerateHelpText(env, sort=lambda x, y: x > y))
...
if env.GetOption("clean") or env.GetOption("help"):
    ...
    keys = list(map(lambda x: (x[0],x[2]), boolopts)) + list(map(lambda x: (x[0],x[2]), nonboolopts)) + list(map(lambda x: (x[0],x[2]), pathopts))
    keys.sort()
...
clean = env.Clean(build,
          list(map(glob.glob,("*.[oa]", "*.[1358]", "*.os", "*.os.*", "*.gcno", "*.pyc", "gps/*.pyc", "TAGS", "config.log"))) + testprogs + \
          generated_sources + list(base_manpages.keys()) + \
          list(map(lambda f: f[:-3], templated)))
